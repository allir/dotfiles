...
The `install` script detects platform (Darwin/Linux), requests sudo upfront with a keep-alive loop, then runs setup scripts from `libexec/` in sequence. On macOS: Xcode CLI tools → Homebrew → Brewfile → shell registration → symlinks → SSH → iTerm2 → Dock → Terminal → bin scripts. Linux runs a subset (sudo → symlinks → SSH).
...
**Symlink strategy (`libexec/setup_symlinks.sh`):** All dot-prefixed files/dirs at repo root (except `.git`, `.gitignore`) get symlinked to `$HOME`. Non-dot root files are also symlinked. Scripts in `bin/` are symlinked to `~/.local/bin`.
...
- Shell scripts use bash (`#!/usr/bin/env bash` or `#!/bin/bash`)
- Aliases and functions shared between bash/zsh go in `.shell/commonrc`
- Shell-specific config (fzf integration) lives in `.shell/.fzf.bash` / `.shell/.fzf.zsh`
